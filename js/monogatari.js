...
$_ready(function () {
	...
	const typedConfiguration = {
		...
		onStringTyped: function () {
			// console.log('string typed, autoplay is', autoPlay, 'autoplay speed', settings.AutoPlaySpeed * 1000)
			if (autoPlay !== null) {
	...
	$_("[data-action='set-auto-play-speed']").on("change mouseover", function () {
		...
		// console.log('speed value, ', value, 'settings ', settings.AutoPlaySpeed)
		Storage.set("Settings", JSON.stringify(settings));
	});
	...
	$_("[data-action]").click(function (event) {
		switch ($_(this).data("action")) {
			...
			// case "open-bag": 
			// 	if ($_("[data-ui='bag']").hasClass("closed")){
			// 		$_("[data-ui='bag']").removeClass("closed")
			// 		$_("[data-ui='bag']").addClass("active");
			// 		if ($_("[data-action='open-bag']").hasClass("flashing")) {
			// 			$_("[data-action='open-bag']").removeClass("flashing")
			// 		}
			// 	}
			// 	break;
			...
			case "toggle-bag":
				...
				else if ($_("[data-ui='bag']").hasClass("active")) {
					$_("[data-ui='bag']").removeClass("active")
					$_("[data-ui='bag']").addClass("closed");
				} 
				break

			case "get-item":
				let item = $_(this).collection[0].id === "map" ? "map" : "article";
				if ($_("[data-ui='" + item + "']").hasClass("closed")){
					$_("[data-ui='" + item + "']").removeClass("closed")
				}
				$_("[data-ui='" + item + "']").addClass("active");
				if ($_("#"+item).hasClass("glowing")) {
					$_("#"+item).removeClass("glowing")
				}
				$_("#" + item).hide();
				break;

			case "close-bag":
				if ($_("[data-ui='bag']").hasClass("active")) {
					$_("[data-ui='bag']").removeClass("active")
					$_("[data-ui='bag']").addClass("closed");
				};
				if (canProceed()) {
			...
			case "toggle-item":
				let item2 = $_(this).hasClass("map") ? "map" : "article"
				if ($_("[data-ui='" + item2 + "']").hasClass("active")) {
					$_("[data-ui='" + item2 + "']").addClass("closed");
					$_("[data-ui='" + item2 + "']").removeClass("active");
				} else {
					$_("[data-ui='" + item2 + "']").removeClass("closed");
					$_("[data-ui='" + item2 + "']").addClass("active");
				}
				break;

			case "select-drink":
				
				$_(".container").addClass("shaking");
				$_(".drinks").style("animation", "none");
				$_("#top-compartment").addClass("fridgeClose");
				$_(".kitchen-furniture").addClass("shaking")
				setTimeout(function(){
					$_("[data-ui='3d-background']").addClass("earthquakeWalk");
					$_("[data-ui='3d-background']").removeClass("kitchenWalk");
				 }, 1500);
	...
	function canProceed () {
		if (!$_("[data-ui='choices']").isVisible()
			&& $_("#game").isVisible()
			&& !$_("[data-ui='map']").hasClass('active')
			&& !$_("[data-ui='article']").hasClass('active')
			&& !$_("#article").hasClass('glowing')
			&& !$_("[data-action='open-bag']").hasClass("flashing")
	...
	function hideGameElements () {
		...
		$_("[data-component='button-container']").removeClass("active");
		$_("[data-ui='map']").removeClass("closed");
